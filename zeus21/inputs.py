...
class Cosmo_Parameters_Input:
    ...
    def __init__(self, omegab=0.0223828, omegac=0.1201075, h_fid=0.67810, As=2.100549e-09, ns=0.9660499, tau_fid=0.05430842, kmax_CLASS=500.0, zmax_CLASS=50.0, Flag_emulate_21cmfast=False):
...
class Cosmo_Parameters:
    ...
    def __init__(self, CosmoParams_input, ClassCosmo):
        ...
        self.mu_baryon = (self.f_H + self.f_He * 4.0) * 0.94  # mproton ~ 0.94 GeV
        ...
        self._ztabinchi = np.linspace(0.0, 100.0, 10000)  # cheap so do a lot
        ...
        _ztabingrowth = np.linspace(0.0, 100.0, 2000)
        ...
        if self.Flag_emulate_21cmfast == True:
            ...
            self.Rsmmax = 500.0  # same as R_XLy_MAX in 21cmFAST. Too low?
...
class Astro_Parameters:
    ...
    def __init__(self, Cosmo_Parameters, astromodel=0, epsstar=0.1, alphastar=0.5, betastar=-0.5, Mc=3e11, fesc10=0.1, alphaesc=0.0, L40_xray=3.0, E0_xray=500.0, alpha_xray=-1.0, Emax_xray_norm=2000, Nalpha_lyA=9690, Mturn_fixed=None, FLAG_MTURN_SHARP=False, accretion_model=1):
        ...
        self.Emax_xray_integral = 10000.0  # max energy in eV that we integrate up to. Higher than Emax_xray_norm since photons can redshift from higher z
        ...
        self.dlogEnergy = (self._log10EMAX_INTEGRATE - self._log10EMIN_INTEGRATE) / (self.Nen_xray - 1.0) * np.log(10.0)  # to get dlog instead of dlog10
