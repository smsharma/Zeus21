...
class Power_Spectra:
    ...
    def __init__(self, Cosmo_Parameters, ClassCosmo, Correlations, T21_coefficients, RSD_MODE=1):
        ...
        self._betaxa = 1.0 / (1.0 + T21_coefficients.xa_avg) / T21_coefficients.xa_avg  # multiplies \delta x_a [again not \delta xa/xa]
        ...
        if Cosmo_Parameters.Flag_emulate_21cmfast == True:
            ...
            _zlisthighz_ = np.linspace(T21_coefficients.zintegral[-1], 99.0, 100)  # beyond z=100 need to explictly tell CLASS to save growth
            _dgrowthhighz_ = cosmology.dgrowth_dz(Cosmo_Parameters, _zlisthighz_)
            _hizintegral = np.trapz(cosmology.Tadiabatic(Cosmo_Parameters, _zlisthighz_) / (1 + _zlisthighz_) ** 2 * _dgrowthhighz_, _zlisthighz_)

        self._betaTad_ = -2.0 / 3.0 * _factor_adi_ / self._lingrowthd * (np.cumsum(_integrand_adi[::-1])[::-1] + _hizintegral)  # units of Tk_avg. Internal sum goes from high to low z (backwards), minus sign accounts for it properly so it's positive.
