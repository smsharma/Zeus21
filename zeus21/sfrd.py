    ...
    def __init__(self, Cosmo_Parameters, ClassCosmo, Astro_Parameters, HMF_interpolator, zmin=5.0):
        ...
        for izp, zp in enumerate(self.zintegral):
            ...
            for n_rec in range(constants.n_max_recycle - 1):
                ...
                nu_lineRR = nu_linezp * (1.0 + self.ztabRsmoo[izp]) / (1 + zp)
        ...
        _xiHirata = pow(_tau_GP * 1e-7, 1 / 3.0) * pow(self.Tk_avg, -2.0 / 3)
        ...
        self.niondot_avg *= Astro_Parameters.N_ion_perbaryon / cosmology.rho_baryon(Cosmo_Parameters, 0.0)  # note the rhob(z=0) because it's comoving volume
...
def Matom(z):
    ...
    return 3.3e7 * pow((1.0 + z) / (21.0), -3.0 / 2)
...
def Salpha_exp(z, T, xe):
    ...
    tau_GP_noreio = 3e5 * pow((1 + z) / 7, 3.0 / 2.0) * (1 - xe)
    gamma_Sobolev = 1.0 / tau_GP_noreio
    return np.exp(-0.803 * pow(T, -2.0 / 3.0) * pow(1e-6 / gamma_Sobolev, -1.0 / 3.0))
